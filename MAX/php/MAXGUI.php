...
function buildMAXAdminDashboardPanel($Type, $Alias)
{
	
	if($Type == 'Registration')
	{
		$Count = getMAXSignUp();
		$Signup = getCurrentMAXSignUpRound();
	}
	else
	{
		$Count = getMAXActiveRound();
	...
	if($Count > 0)
	{
		if($Type == 'Registration')
		{
			$CurrentMAXRound = getCurrentMAXSignUpRound();
		}
		else
		{
		...
		print("<h3>MAX round Status: ".$Type."</h3>");
		print("<div id='".$Type."MAXInfo'>");
		print("<div id='".$Type."MAXRoundStart'><strong>Date Started:</strong> $StartDate</div>");
		print("<div id='".$Type."MAXRoundSignUpsClose'><strong>Sign Ups close:</strong> $SignUpClose</div>");
		print("<div id='".$Type."MAXRoundFinalSubmissionDate'><strong>Final date for submissions:</strong> $EndDate</div>");
		print("<div id='".$Type."MAXRoundTheme'><strong>Theme:</strong> $Theme</div>");
		...
		elseif($SignUpCloseComparable < $now)
		{
			...
			else
			{
				...
				else
				{
					...
					print("<div id='MatchUpList".$MAXID."'></div>");
				}
...
function buildMAXAdminDashboard($Alias)
{
	...
	print("<div class='MAXRoundDashboard'>");	
	
	buildMAXAdminDashboardPanel("Registration", $Alias);
	buildMAXAdminDashboardPanel("Active",$Alias);
...
function buildMaxPanel($Type, $Alias)
{
	if($Type == 'Registration')
	{
		$Count = getMAXSignUp();
	...
	if($Count > 0)
	{
		...
		$now = time()*1000;
		print("<div id='MAX'".$Type."'>");
		print("<div id='MAX".$Type."Header'><h2>Sign-Ups</h2></div>");
		print("<div id='currentMAX".$Type."Round'>");
		print("<h3>MAX Sign-Up</h3>");
		print("<div class='MAXStart'>Start Date: $StartDate</div>");
		print("<div id='MAX".$Type."Closes'>Sign-ups closed: $CloseDate</div>");
		print("<div id='MAX".$Type."SubmissionDeadline'>Submission Deadline: $EndDate</div>");
		if($DeadlineComparable < $now)
		{
			print("<div id=MAX".$MAXID."RoundClosed'>This MAX Round is now over.</div>");
		...
		{
			print("<div id='".$MAXID."Closed'>Sign ups are currently closed.</div>");
			if(checkIfUserIsSignedUp($Alias, $MAXID))
			{
				if(isMAXRoundLocked($MAXID))
				{
		...
		{
			print("<div id='".$MAXID."Open'>");
			if(checkIfUserHasACharacterWithReferences($Alias))
			{
				if(checkIfUserIsSignedUp($Alias, $MAXID))
				{
					print("<div id='".$MAXID."signMeUp'>");
					print("<input type='button' value='Withdraw' id='".$Type."withdrawFromRoundButton' onclick=\"withdrawUserFromMAXRound('$Alias','$MAXID')\">");
					print("<input type='button' value='Sign Up!' id='".$Type."ForRoundButton' onclick=\"signUserUpForMAXRound('$Alias','$MAXID')\" style=\"display: none;\">");
					print("</div>");
				}
				else
				{
					print("<div id='".$MAXID."signMeUp'>");
					print("<input type='button' value='Sign Up!' id='".$Type."ForRoundButton' onclick=\"signUserUpForMAXRound('$Alias','$MAXID')\">");
					print("<input type='button' value='Withdraw' id='".$Type."withdrawFromRoundButton' onclick=\"withdrawUserFromMAXRound('$Alias','$MAXID')\" style=\"display: none;\">");
					print("</div>");
				}
				print("<div id='".$MAXID."ERRMSG' class='ERRMSG'></div>");
				print("</div>");//end SignupsOpen
				print("<script>");
				print("$('#".$MAXID."withdrawFromRoundButton').click
				(
					function()
					{
						$('#".$MAXID."withdrawFromRoundButton').hide();
						$('#".$MAXID."ForRoundButton').show();
				});");
				print("$('#".$MAXID."signUpForRoundButton').click
				(
					function()
					{
						$('#".$MAXID."withdrawFromRoundButton').show();
						$('#".$MAXID."ForRoundButton').hide();
				});");
				print("</script>");
			}
			else
			{
				...
				print("<form action='?' method='GET'>");
				print("<input type='submit' name='submit' id='".$MAXID."ManageProfile' value='Manage Profile' class='MAXControlPanelButton'> ");
				print("</form>");
				print("</div>");
			}
		}
	...
	{
		print("<div id='noCurrentMAXRound'>");
...
{
	buildMAXSplash();
	buildMaxPanel("Registration", $Alias);
	buildMaxPanel("Active", $Alias);
	...
	if(doesUserHaveAnyRoundsIncomplete($Alias))
	{
		...
		foreach($RoundList->getRecords() as $Round)
		{
...
function buildAddMAXRound()
{
	...
	print("<option value='Registration'>Registration</option>");
...
function buildMAXCharactersOfRecipient($Alias)
{
	...
	if(getCharactersOfUserCount($Alias) > 0)
	{
		...
		foreach($CharacterList->getRecords() as $Character)
		{
			...
			print("<div id='Character$CharacterID' class='characterWrap charactercard col-12 col-sm-6 col-md-4'>");
			$preferredURL = getCharacterThumbnail($CharacterID);
			if($preferredURL != '')
			{
				$preferredURL = str_replace(' ', '%20', $preferredURL);
				print("<div class='card'><a href='https://www.comicadia.com/MAX/index.php?CharacterID=$CharacterID&Fields=Characters' style='background-image: url(\"$preferredURL\");'><h3 class='card-title'>$CharacterName</h3></a></div>");
			}
			else
			{	
				print("<div class='card'><a href='https://www.comicadia.com/MAX/index.php?CharacterID=$CharacterID&Fields=Characters' style='background-image: url(\"https://www.comicadia.com/media/user.png\");'><h3 class='card-title'>$CharacterName</h3></a></div>");
			}
			if(isCharacterCurrentlyPreferred($CharacterID))
			{
				print("<span id='preferredCharacter' class='preferredCharacterIndicator'>PREFERRED</span>");
			}
			/*
	...
	print(" <div class='clear'></div></div> <!-- End RecipientCharacters -->"); //end RecipientCharacters
}
...
function buildMAXCharacterProfilWithReferences($CharacterID)
{
	...
	if($ThumbnailURL != '')
	{
		$ThumbnailURL = str_replace(' ', '%20', $ThumbnailURL);
	...
	if(getCountOfMAXRoundsUserParticipatedIn($Alias) > 0)
	{
		...
		foreach($UserParticipatedRounds->getRecords() as $Round)
		{
			...
			if(checkIfReceiverSubmittedTheirEntryForMAXRoundBasedOnEntryID($EntryID, $Receiver))
			{
				...
				print("<div class='previousRoundPreview charactercard col-12 col-sm-6 col-md-4'><div class='card'>");
				print("<a href='https://www.comicadia.com/MAX/index.php?EntryID=$EntryID' target='_blank'  style='background-image: url($imgURL)'>");
				print("<span class='preferredCharacterIndicator'><small style='color: black'>$MaxStartDate - $MAXEndDate</small></span>");
				print("<h3 class='card-title'>$CharName</h3></a>");
				print("</div></div>"); // end PreviousRoundPreview
			}
			else
...
function buildMAXCharactersPanelFromList($CharacterList)
{
	...
	foreach($CharacterList->getRecords() as $Character)
	{
		...
		print("<div class='charactercard col-12 col-sm-6 col-md-4'>");
		print("<div class='card'>");
		print("<a href='https://www.comicadia.com/MAX/index.php?CharacterID=$CharacterID"."&Fields=Characters' style='background-image: url(");
		$PreferredThumbnail = getCharacterThumbnail($CharacterID);
		if($PreferredThumbnail != '')
		{
			$PreferredThumbnail = str_replace(' ', '%20', $PreferredThumbnail);
			print("$PreferredThumbnail)'>");
		}
		else
		{
			print("https://www.comicadia.com/media/user.png)'>");
		}
		print("<h3 class='card-title'>$Name</h3></a><!-- end card-title classed div -->");
...
function buildAdoptionWaitingReview($Alias)
{
	...
	foreach($AdoptionsForReview->getRecords() as $Orphan)
	{
		...
		if($RequestedCharacterID != '')
		{
			...
			{
				$RequestedCharThumbnail = str_replace(' ', '%20', $RequestedCharThumbnail);
